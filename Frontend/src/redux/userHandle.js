...
export const authUser = (fields, role, mode) => async (dispatch) => {
    ...
    try {
        const result = await axios.post(`http://localhost:5000/${role}${mode}`, fields, {
...
export const getProductsbySeller = (id) => async (dispatch) => {
    ...
    try {
        const result = await axios.get(`http://localhost:5000/getSellerProducts/${id}`);
        if (result.data.message) {
...
export const getProductDetails = (id) => async (dispatch) => {
    ...
    try {
        const result = await axios.get(`http://localhost:5000/getProductDetail/${id}`);
        if (result.data.message) {
